...
import {GETSEARCHBYNAME,LOGOUT} from  '../../src/actions'
...
import FilterMenu from './FiletrMenu'
...
export default function PrimarySearchAppBar() {
  const { cart, updateCartQuantity, removeAllCartProduct } = React.useContext(CartContext);
...
*/
  ...
  const renderMobileMenu = (
    ...
    >
      ...
      {isLogged && <MenuItem onClick={async ()=>{
     ///Cookie.remove('cart')
     await Cookie.set('cart', JSON.stringify( "" ))
      dispatch(LOGOUT())
      //cart.map( (product:any) => (removeAllCartProduct( product )) 
      navigate('/')
      await removeAllCartProduct( cart[0] )
