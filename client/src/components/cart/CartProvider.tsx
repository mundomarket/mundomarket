...
export const CartProvider: FC<Props> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      const cookieProducts = Cookie.get("cart")
        ...
        : []; // Cookie.get(cart) pregunto si existe para que no sea undefined, lo parseo sino array vacio
      dispatch({
  ...
  useEffect(() => {
    //esto hace que se mantengan los productos en  el carrito si actualizo la pagina
  ...
  const addProductToCart = (product: ICartProduct) => {
    ...
    const updatedProducts = state.cart.map((p) => {
      //si el producto agregado esta en el carrito entonces mapea el carrito
